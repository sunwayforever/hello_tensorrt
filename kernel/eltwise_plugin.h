...
class EltwisePluginCreator : public IPluginCreator {
   public:
    const char* getPluginName() const noexcept override { return "ELTWISE"; }
    const char* getPluginVersion() const noexcept override { return "1"; }
    ...
    IPluginV2* createPlugin(
        ...
        auto* plugin = new EltwisePlugin(*fc);
        mFieldCollection = *fc;
        mPluginName = name;
        return plugin;
    }
    IPluginV2* deserializePlugin(
        ...
        auto* plugin = new EltwisePlugin(serialData, serialLength);
        mPluginName = name;
        return plugin;
    }
    void setPluginNamespace(const char* libNamespace) noexcept override {
